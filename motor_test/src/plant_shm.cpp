...
uint16 TXPDO_ADDR_GTWI[4] = {3, 0x1A02, 0x1A03, 0x1A1E};     // {Number of entry to be mapped, Entry address, Entry address, ...}
...
char *IFNAME = "enp6s0"; // Name of IF card
char IOmap[4096];
...
void *realtime_thread(void *arg)
{
    ...
    double      target_current[NUMOFSLAVES]; std::fill_n(target_current,NUMOFSLAVES,0.0);
    double      target_velocity[NUMOFSLAVES];   std::fill_n(target_velocity,NUMOFSLAVES,0.0);

    double      target_position[NUMOFSLAVES];   std::fill_n(target_position,NUMOFSLAVES,0.0);
    ...
    std::cout  << "paass" << ec_init(IFNAME)<<std::endl;
    if(ecat_init(IFNAME))       //Initialize EtherCAT Master
        sigMainKill = 0;
    else
    {
        sigMainKill = 1;
        std::cout << "canno"<<std::endl;
    }
    ...
    while(!sigMainKill) // cnt가 어느정도 이상되면 main에서 sigMainkill을 true로 바꿔줌
    {
        ...
        std::cout <<"position1= "<< position_raw[0] << std::endl;
        // ec_send_processdata();
        // wkc = ec_receive_processdata(EC_TIMEOUTRET);
        std::cout <<"position2 = "<< position_raw[0] << std::endl;
        // for (int i=1 ; i<=NUMOFSLAVES ; i++)
        // {
        //     slave_state[i] = ec_slave[i].state;
        // }
        ...
        std::cout <<"position3 = "<< position_raw[0] << std::endl;

        // for (int i = 0 ; i < NUMOFSLAVES ; i++) //Copy the data from received PDO
        // {
            // position_raw[0]     = in_twitter[0]->TXPDO_ACTUAL_POSITION_DATA;
        ...
        // }
        // std::cout <<"position 4= "<< position_raw[0] << std::endl;
        for (int i = 0 ; i < NUMOFSLAVES ; i++) //Copy the data from received PDO
        {
            ...
            // out_twitter[i]->RXPDO_TARGET_VELOCITY_DATA     = (int32)(target_velocity[i]*20000.0/(2*PI));
